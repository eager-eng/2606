...
def 运行单因素情景(
    ...
    起点统计 = pd.DataFrame()
    if 统一初始能源 is None:
        统一初始负荷 = 重构联合负荷(统一初始任务, 基准时间数据, GPU信息, 功率映射)
        最低碳起点, _, 最低碳记录 = 求解能源模型(
            统一初始负荷,
            储能信息,
            "单因素情景最低碳起点",
            主目标="碳排放",
        ...
        if 弃电整数起点 is None:
            raise RuntimeError("单因素情景未得到统一整数起点")
        统一初始能源, _, 固定模式记录 = 求解能源模型(
            统一初始负荷,
            储能信息,
            "单因素情景固定模式可行起点",
            主目标="成本",
        ...
        起点统计 = pd.DataFrame([最低碳记录, 弃电起点记录, 固定模式记录])
        if 统一初始能源 is None:
            raise RuntimeError("单因素情景未得到统一可行能源起点")
    所有结果 = []
    for 名称, 时间数据, 碳上限, 弃电上限 in 情景设置:
        任务, 负荷, 能源, 指标, 迭代, 迁移, 统计 = 协调任务与能源(
            ...
            碳上限,
            弃电上限,
            初始暖启动=统一初始能源 if "新能源波动" not in 名称 else None,
            最大迭代=统一协调迭代数,
            候选池上限=10000,
            输出日志=输出日志,
            时间限制_s=时间限制_s,
        ...
        if len(起点统计):
            统计 = pd.concat([起点统计, 统计], ignore_index=True)
            起点统计 = pd.DataFrame()
...
def 保存图形(图形, 图片目录, 文件名):
    PNG路径 = 图片目录 / f"{文件名}.png"
    PDF路径 = 图片目录 / f"{文件名}.pdf"
    for 目标路径, 格式, 参数 in [
        (PNG路径, "png", {"dpi": 300}),
        (PDF路径, "pdf", {}),
    ]:
        临时文件描述符, 临时路径 = tempfile.mkstemp(prefix="figure_", suffix=f".{格式}", dir=图片目录)
        os.close(临时文件描述符)
        try:
            with open(临时路径, "wb") as 文件:
                图形.savefig(文件, format=格式, bbox_inches="tight", **参数)
...
采用三目标ε约束 Pareto、Gurobi 能源储能 MILP、固定整数状态 LP 条件影子价格和 EDF 优先成本感知列表调度。问题二任务方案作为初始解，问题三储能轨迹未直接复制。结果文件记录的本次运行耗时 {运行时间_s:.2f} s。
任务数据使用第0—2399小时实际到达任务，第2400—2405小时仅用于结清末端任务，第2406小时不安排AI任务，仅保留固定负荷和储能结算。
固定任务负荷下得到的五个非支配点均从同一问题二任务方案出发，分别进行{统一协调迭代数}轮任务—能源联合协调；协调完成后重新筛选非支配解并选择理想点折中方案。基准与六个单因素情景均从同一问题二任务方案出发并采用统一迭代上限；非新能源波动情景共用可行能源暖启动，新能源波动情景按各自新能源数据重新求取可行起点。
...
def 主程序():
    ...
    if 参数.scenarios_only:
        ...
        Pareto协调结果映射 = {}
        Pareto协调迭代 = 安全读取CSV(输出目录 / "迭代记录.csv")
        if len(Pareto协调迭代):
            Pareto协调迭代 = Pareto协调迭代[
                Pareto协调迭代["Scenario"].astype(str).str.endswith("_联合协调")
            ].copy()
        Pareto协调迁移 = 安全读取CSV(输出目录 / "任务调整明细.csv")
        if len(Pareto协调迁移):
            Pareto协调迁移 = Pareto协调迁移[
                Pareto协调迁移["Scenario"].astype(str).str.endswith("_联合协调")
            ].copy()
        既有Pareto检验 = 安全读取CSV(输出目录 / "约束检验.csv")
        if len(既有Pareto检验):
            既有Pareto检验 = 既有Pareto检验[
                既有Pareto检验["Scenario"].astype(str).str.endswith("_联合协调")
            ].copy()
        情景名称 = ["单因素情景", "宽松碳约束", "严格碳约束", "平坦电价", "扩大峰谷价差", "低新能源波动", "高新能源波动"]
        Pareto统计 = Pareto统计[
            ~Pareto统计["方案"].astype(str).str.startswith(tuple(情景名称))
        ].copy()
